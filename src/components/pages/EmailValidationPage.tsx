...
export function EmailValidationPage({ onComplete, onBack }: EmailValidationPageProps) {
  const [email, setEmail] = useState("");
  const [isVerifying, setIsVerifying] = useState(false);
  const [verificationStep, setVerificationStep] = useState<"input" | "verify" | "success" | "confirm">("input");
  ...
  useEffect(() => {
    ...
    const tokens = [cilogon, orcid].filter(token => 
      ...
      <Card className="shadow-lg border-2 border-[rgb(120,176,219)] bg-[rgb(255,255,255)]">
        <CardHeader className="pb-8 text-center">
          ...
          <CardDescription className="text-lg mt-2 text-[rgb(50,135,200)]">
            {verificationStep === "input" && "Please verify your email address to continue"}
            {verificationStep === "verify" && "Enter the verification code sent to your email"}
            {verificationStep === "confirm" && "Please confirm this is your email address"}
            ...
            <Alert className="border-2 border-[rgb(120,176,219)] bg-[rgb(236,244,250)]">
              <AlertDescription className="text-base text-[rgb(64,143,204)]">
                <div className="space-y-2">
                  <div><strong>Authenticated as:</strong> {userClaims.sub || 'Unknown user'}</div>
                  <div><strong>Provider:</strong> {userToken?.provider?.toUpperCase()}</div>
                  {userClaims.email && (
                    <div><strong>Email from token:</strong> {userClaims.email}</div>
                  )}
                </div>
              </AlertDescription>
            </Alert>
          ...
          {verificationStep === "input" && (
            <div className="space-y-6">
              <div className="space-y-3">
                <label htmlFor="email" className="text-base font-semibold text-[rgb(64,143,204)]">
                  Email Address
                </label>
                <Input
                  id="email"
          ...
          {verificationStep === "verify" && (
            <div className="space-y-6">
              <Alert className="border-2 border-[rgb(120,176,219)] bg-[rgb(236,244,250)]">
                <AlertDescription className="text-base text-[rgb(64,143,204)]">
                  <div className="space-y-2">
                    <div><strong>Verification code sent to:</strong> {email}</div>
              ...
              <div className="space-y-3">
                <label htmlFor="code" className="text-base font-semibold text-[rgb(64,143,204)]">
                  Verification Code
                </label>
                <Input
                  id="code"
          ...
          {verificationStep === "confirm" && (
            <div className="space-y-6">
              <Alert className="border-2 border-[rgb(120,176,219)] bg-[rgb(236,244,250)]">
                <AlertDescription className="text-base text-[rgb(64,143,204)]">
                  <div className="space-y-2">
                    <div><strong>Email address from your identity provider:</strong></div>
                    <div className="text-lg font-mono bg-white p-3 rounded border">{email}</div>
                    <div className="text-sm opacity-70">
                      This email was verified by your identity provider. Is this correct?
                    </div>
                  </div>
                </AlertDescription>
              </Alert>
            ...
            <div className="space-y-6 text-center">
              <Alert className="border-2 border-green-200 bg-green-50">
                ...
                <AlertDescription className="text-base ml-2 text-green-800">
                  <div className="space-y-2">
                    <div><strong>Email verified successfully!</strong></div>
                    <div>Your email <strong>{email}</strong> has been confirmed.</div>
                    <div className="text-sm opacity-70">Redirecting to your dashboard...</div>
                  </div>
                </AlertDescription>
              </Alert>
